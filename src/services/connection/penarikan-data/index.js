...
const actions = {
  async getDataReportVisit({ commit, state }) {
    ...
    try {
      const { data } = state
      let startDate = moment(data.body_report_visit.start_date).format('DD-MM-YYYY')
      let endDate = moment(data.body_report_visit.end_date).format('DD-MM-YYYY')
  ...
  async getDataReportHasilSurvey({ commit, state }) {
    ...
    try {
      ...
      const result = await apiClientSIG.post(`ReportCRMRevamp/ReportHasilSurvey`, formBody)
